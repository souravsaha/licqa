...
for i in range(150):
    ...
    for tag_items in filtered_tag_list:
        ...
        max_cosine_value = -1 
        total_cosine_value = 0
        str_sent = ""
        #print(ent_sentence_map[tuple_val])
        sentence_list = ent_sentence_map[tuple_val]
        # Option 1: take max of cosine value (giving best result)
        ...
        """
        # Option 2: take avg cosine value
        for sentence in sentence_list:
            score = cosine(model.encode([str.lower(question)])[0], model.encode([str.lower(sentence)])[0])
            total_cosine_value += score
            print(tuple_val, sentence, score)
            #if cosine_value > max_cosine_value :
            #    max_cosine_value = cosine_value
            #    str_sent = str(sentence)
        max_cosine_value = total_cosine_value / len(sentence_list)         # avg cosine value -> max_cosine_value, name is little ambiguous as of now
        print(max_cosine_value, len(sentence_list), tuple_val)
        """
        if max_cosine_value != -1 :
            # similarity measure : taking doc freq into account, score = 10/doc_freq + max_cosine_value; don't want to rerank change this, 2-3 lines
            ...
            #score_tag_tuple = ((doc_freq * doc_number * .8) + (0.2 * max_cosine_value), tuple_val)
...
with open(resultPathStart + samplingType + "foo_foo_"  + quesType + ".json" , "w+") as outfile: 
